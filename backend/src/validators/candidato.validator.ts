...
export const registrarCandidatoSchema = z.object({
  body: z.object({
    ...
    correo: z.union([z.literal(''), z.string().email('Correo inválido')]).optional().nullable(),
    ...
    estatura: z.union([z.literal(''), z.number().positive().max(3)]).optional().nullable(),
    peso: z.union([z.literal(''), z.number().positive().max(500)]).optional().nullable(),
    licencia: z.enum(['S', 'N']).default('N'),
    cat_licencia: z.string().optional().nullable(),
    vehiculo: z.enum(['S', 'N']).default('N'),
    marca: z.string().optional().nullable(),
    ano_vehic: z.union([z.literal(''), z.number().int().min(1900).max(new Date().getFullYear() + 1)]).optional().nullable(),
