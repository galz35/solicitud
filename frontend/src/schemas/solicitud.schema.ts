...
export const datosGeneralesSchema = z.object({
  ...
  estatura: z.union([z.literal(''), z.coerce.number().positive('Debe ser número positivo').max(3, 'Estatura inválida')]).optional().nullable(),
  peso: z.union([z.literal(''), z.coerce.number().positive('Debe ser número positivo').max(500, 'Peso inválido')]).optional().nullable(),
  licencia: z.enum(['S', 'N']).default('N'),
  cat_licencia: z.string().optional().nullable(),
  vehiculo: z.enum(['S', 'N']).default('N'),
  marca: z.string().optional().nullable(),
  ano_vehic: z.union([z.literal(''), z.coerce.number().int().min(1900).max(new Date().getFullYear() + 1)]).optional().nullable(),
